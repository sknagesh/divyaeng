...
if($adddel=='add')
{
	...
	while($j<$nooftools)
	{
		$Tool_ID_1[$j]=$tlist[$k];
		$k++;
		$Insert_ID1[$j]=$tlist[$k];
		$k++;
		$Tool_ID_2[$j]=$tlist[$k];
		$k++;
		$Insert_ID2[$j]=$tlist[$k];
		$k++;
		$Holder_ID[$j]=$tlist[$k];
		$k++;
		...
		while($j<$nooftools	)
		{
			$query="INSERT INTO Ope_Tool (Operation_ID,Tool_ID_1,Insert_ID_1,Tool_ID_2,,Insert_ID_2,Holder_ID,Ope_Tool_Desc,Ope_Tool_OH,Ope_Tool_Life) 
			VALUES('$opid','$Tool_ID_1[$j]','$Insert_ID1[$j]','$Tool_ID_2[$j]','$Insert_ID2[$j]','$Holder_ID[$j]','$Ope_Tool_Desc[$j]','$Ope_Tool_OH[$j]','$Ope_Tool_Life[$j]');";
...
}else if($adddel=='del')
{
	...
	foreach ($del as $key => $value) {
		$q="DELETE FROM Ope_Tool WHERE Ope_Tool_ID=$value;";
