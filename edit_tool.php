...
print("<p><label for=\"tooltype\">Tool Type</label>");
	...
	{
		if($rro['Tool_Type_ID']==$row['Tool_Type_ID']){$sel="Selected=\"Selected\"";}else{$sel="";}
...
print("<p><label for=\"tmake\">Tool Manufacturrer</label>");
   ...
   $qq="select * from Tool_Brand;";
  $rr = mysql_query($qq, $cxn) or die(mysql_error($cxn));
  print("<select name=\"Brand_ID\" id=\"Brand_ID\" class=\"required\">");
  print("<option value=\"\" >Select Tool Brand</option>");
  while($rro = mysql_fetch_assoc($rr))
  {
    if($rro['Brand_ID']==$row['Brand_ID']){$sel="Selected=\"Selected\"";}else{$sel="";}
    print("<option value=".$rro['Brand_ID']." $sel >");
    print("$rro[Brand_Description]</option>");
  }
print("</select>");
...
print("<p><label for=\"tcr\">Corner Radius</label>
     <input id=\"tcr\" name=\"tcr\" size=\"25\"  class=\"number\" value=\"$row[Tool_Corner_Rad]\"/>
  </p>");

print("<p><label for=\"tcr\">CuttingEdge Angle</label>
     <input id=\"tang\" name=\"tang\" size=\"25\"  class=\"number\" value=\"$row[Tool_Corner_Rad]\"/>
  </p>");
