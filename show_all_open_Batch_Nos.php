...
$query="SELECT EX_Challan_NO,DATE_FORMAT(EX_Challan_Date,'%d/%m/%Y') as cdate,Purchase_Ref,
		...
		Mfg_Batch_NO,Batch_Remarks,Heat_Code,DATE_FORMAT(Commited_Date,'%d/%m/%Y') as comitdate,
...
print("<tr class=\"t\"><th>Drawing NO and Name</th><th>Challan No</th><th>Challan Date</th><th>Purchase Ref</th>
			<th>Purchase Ref Date</th><th>Quantity Received</th><th>Qty Used In Batch</th><th>Batch No</th><th>Heat Code</th><th>Commitment Date</th><th>Batch Remarks</th></tr>");

	while($row = mysql_fetch_assoc($resa))
	{
		if($row['cdate']=='00/00/0000'){$edt='';}else{$edt=$row['cdate'];}
		if($row['prf']=='00/00/0000'){$prf='';}else{$prf=$row['prf'];}
		if($row['comitdate']=='00/00/0000'){$cd='';}else{$cd=$row['comitdate'];}
